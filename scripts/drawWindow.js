(function () {
	...
	function el(dom, onready) {
	  ...
	  this.children = Array.from(dom.children)
		.filter((child) => !child.classList.contains('drawWindow-ignore'))
		.map((child) => new el(child))
		.sort(orderByZIndex);
	...
	el.prototype.initializeDOM = function () {
	  ...
	  this.boundingClientRect = getScrolledRects(
	  ...
	  var noBorders = (this.boundingClientRectNoBorders = Object.assign(
		{},
	  ...
	  if ((scrollTop > 0 || scrollLeft > 0) && this.body != this) {
		this.scrollParent = this;
	  }
  
	  this.shouldRender = dom.style.display !== 'none';
	  ...
	  this.id = dom.id || false;
	  this.src = this.tagName == 'img' ? dom.src : css.backgroundImage;
  
	  css.font =
		`${css.fontStyle} ${css.fontWeight} ${css.fontSize}px ${css.fontFamily}`.trim();
	...
	el.prototype.renderBackground = function (ctx, rect) {
	  ...
	  if (loadedImage) {
		...
		if (repeat == 'no-repeat') {
		  ctx.drawImage(
			retrieveImageFromCache(this.src),
			backgroundRect.left,
			backgroundRect.top,
			backgroundRect.width,
			backgroundRect.height
	...
	el.prototype.renderBorders = function (ctx, rect) {
	  ...
	  if (borderLeftWidth) {
		...
		ctx.fillRect(left, top, borderLeftWidth, height);
	  }
  
	  if (borderTopWidth) {
		ctx.fillStyle = css.borderTopColor;
		ctx.fillRect(left, top, width, borderTopWidth);
	  }
	...
	function retrieveImageFromCache(src) {
	  assert(
		retrieveImage.cache.hasOwnProperty(src),
		'Error: image has not been loaded into cache'
	  );
	...
	function retrieveImage(src, cb, ownerDocument, useBroken) {
	  ...
	  function sendError() {
		...
		img.onload = sendSuccess;
		img.src = useBroken
		  ? retrieveImage.brokenImage
		  : retrieveImage.transparentImage;
	  }
	...
	function getClippingRect(dom, css) {
	  ...
	  var borderLeftWidth = parseInt(css.borderLeftWidth, 10);
	  var borderTopWidth = parseInt(css.borderTopWidth, 10);
	  var borderRightWidth = parseInt(css.borderRightWidth, 10);
	  var borderBottomWidth = parseInt(css.borderBottomWidth, 10);
	  ...
	  if (window.navigator.userAgent.indexOf('MSIE ') > 0) {
		for (var i = 0, len = styles.length; i < len; i++) {
		  ...
		  if (attr == 'outline-width') {
			...
			val = window.getComputedStyle(elem, null).getPropertyValue(attr);
		  }
		...
		for (var i = 0, len = styles.length; i < len; i++) {
		  ret[
			styles[i].replace(/-([a-z])/g, function (g) {
			  return g[1].toUpperCase();
			})
		  ] = computedStyle.getPropertyValue(styles[i]);
		}
	  }
	...
	function assert(isTrue) {
	  if (!isTrue) {
		log('ASSERTION FAILURE', arguments);
	  }
	}
	function log() {
	  if (window.console) {
	...
	function error(msg) {
	  throw '[drawWindow] ' + msg;
	  return false;
	}
  
	retrieveImage.cache = {};
	retrieveImage.transparentImage =
	  'data:image/png;base64,';
  
	retrieveImage.brokenImage =
	  'data:image/png;base64,';
  
	// Kick off rendering if this has been loaded into an iframe
	if (window.parent != window && window.parent.drawWindowReady) {
	  document.addEventListener('DOMContentLoaded', function () {
		if (window.parent != window && window.parent.drawWindowReady) {
		  window.parent.drawWindowReady(window);
		}
	  });
	}
  })();
